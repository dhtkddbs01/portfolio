...
export const projects: Project[] = [
  // ───────────────────────── 실무 ─────────────────────────
  {
    ...
    highlights: [
      ...
      "설계서 전수 검사로 네이밍룰 위반 826건 분류 · 우선순위화",
  ...
  },
  {
    ...
    status: "완료",
    kind: "실무",
    summary: "11개 판매 플랫폼 API · 수기 매출 시트 · 광고 매체비, 세 성격의 원천을 공통 분석 모델로 정규화",
    metric: "원천 3종(API·수기·광고) → 공통 모델 1벌",
    description: [
      "자사몰·오픈마켓·해외마켓·ERP 등 11개 판매 플랫폼 API의 주문·배송·클레임·결제·회원 데이터를 하나의 스키마로 수렴시켰다. 채널마다 필드명도, 통화도, 세금 처리도 달랐다. 여기에 API가 없는 B2B·쿠팡·올리브영 매출과 상품마스터 같은 기초정보는 수기 시트를 S3 → Lambda → Athena로 적재해 같은 모델에 붙였고, MI 광고 매체비와 캠페인·광고 데이터까지 합쳐 매출과 광고비를 한 기준으로 볼 수 있게 했다.",
    ...
    stack: ["dbt", "AWS Athena", "AWS Lambda", "S3", "Glue", "Salesforce Data Cloud", "Marketing Cloud Intelligence", "SQL"],
    highlights: [
      "API·수기·광고 세 성격의 원천을 하나의 dbt 모델 계층으로 — 93개 모델 · 1,430컬럼, 테스트 39건",
      "채널 간 계정 식별자 충돌 해소 — 브랜드 슬롯 포함 해시로 재설계",
      "누락 채널 확장으로 주문 약 38,400건(+17%) 복구",
      "수기 시트 적재 경로(CSV → S3 → Lambda → Athena) 구축, 원천 34폴더 ↔ DC 32 DLO 정합 감사",
